...
int mypipe_register(mypipe_t *ptr,int opmap)
{
	...
	while(me->count_rd <= 0 || me->count_wwr <= 0)
...
static void mypipe_readbyte_unlock(struct mypipe_st *me,char *datap)
{
	if(me->datasize <= 0)
		return -1;
...
int mypipe_read(mypipe_t *ptr,void * buf,size_t count)
{
	...
	while(me->datasize <= 0 && me->count_wr > 0 ) //没有第二个当当前管道空但是没有写者,读者就退出
		pthread_cond_wait(&me->cond,&me->mut);
